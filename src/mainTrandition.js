...
let clock;
let modelsGroup; // Top-level group to hold all models
...
// Generate/rebuild all models (Grid layout: Z is for columns, Y is for rows; wraps to the next Y row if modelsPerRow is exceeded)
function createModels() {
    ...
    // Shared resources
    const geometry = new THREE.TorusGeometry(0.4, 0.15, 1000, 10000); // Can also use Sphere/TorusKnot
    // face
    let faceCount;
    if (geometry.index) {
    ...
    // const geometry = new THREE.SphereGeometry(0.5, 64, 32); // Radius 0.5, 64 width segments, 32 height segments
    ...
    for (let i = 0; i < params.modelCount; i++) {
        ...
        // Grid index: col along Z, row along Y
        ...
        // Generate a different color for each model (body + cap)
        const hue = params.modelCount > 0 ? (i / params.modelCount) % 1 : 0;
        const bodyColor = new THREE.Color().setHSL(hue, 0.7, 0.55); // Body color
        const capColor = new THREE.Color().setHSL(hue, 0.7, 0.55); // Capping plane color

        // (2) Solid mesh -- rendered after this model's stencil
        ...
        // Key: render the solid mesh before the capping plane
        clippedColorFront.renderOrder = i + 1.0;
        modelContainer.add(clippedColorFront);

        // (3) Capping plane -- rendered after the solid mesh (each model also gets a different color for distinction)
        const planeMat = new THREE.MeshStandardMaterial({
            ...
            // polygonOffset: true,
            // polygonOffsetFactor: -2.0, // Adjust as needed (usually 1)
            // polygonOffsetUnits: i*2    // Add a small offset for each i
        });

        const po = new THREE.Mesh(planeGeom, planeMat);
        // Keep this if you want a fixed orientation for the capping plane; to make it follow the local plane normal, see the code in animate()
...
export default function init() {
    ...
    // Single clipping plane (in the -X direction, at x=0)
...
Object.assign(stats.dom.style, {
  ...
  zIndex: 9999
    ...
    gui.add(params, 'modelsPerRow', 1, 50, 1).name('Models Per Row').onChange(createModels); // New GUI control
...
function animate() {
    ...
    for (let i = 0; i < planeObjects.length; i++) {
        ...
        // Transform the global plane to the local coordinate system of po.parent (the model container)
